...
DROP FUNCTION IF EXISTS admin_revoke_all_access(UUID);
DROP FUNCTION IF EXISTS admin_unenroll_user(UUID, UUID);
...
AS $$
BEGIN
    -- 실제 운영환경에서는 profiles 테이블에 role 컬럼을 추가하여 관리
    -- 현재는 임시로 true 반환 (서비스 키 사용 시 관리자로 간주)
    RETURN true;
END;
$$;
...
-- 완료 메시지
SELECT '관리자 RPC 함수들이 profiles 테이블 기준으로 재생성되었습니다.' as status; 